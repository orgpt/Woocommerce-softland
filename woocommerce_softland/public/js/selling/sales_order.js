frappe.ui.form.on('Sales Order', {
	...
	sync_sales_order: function(frm) {
		...
		frappe.call({
			method: "woocommerce_softland.tasks.sync_sales_orders.run_sales_order_sync",
			args: {
	...
	load_shipment_trackings_table: function(frm) {
		...
		frappe.call({
			method: "woocommerce_softland.overrides.selling.sales_order.get_woocommerce_order_shipment_trackings",
			args: {
				doc: frm.doc
			},
			callback: function(r) {
	...
	prompt_user_for_shipment_trackings: function(frm){
		...
		frappe.call({
			method:
				"woocommerce_softland.woocommerce.doctype.woocommerce_server"+
	...
	update_shipment_trackings: function(frm){
		...
		frappe.call({
			method:
				"woocommerce_softland.overrides.selling.sales_order.update_woocommerce_order_shipment_trackings",
			args: {
				doc: frm.doc,
