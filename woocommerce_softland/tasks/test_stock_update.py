...
from woocommerce_softland.tasks.stock_update import (
...
class TestWooCommerceStockSync(FrappeTestCase):
    ...
    @patch("woocommerce_softland.tasks.stock_update.frappe")
    @patch("woocommerce_softland.tasks.stock_update.APIWithRequestLogging", autospec=True)
    def test_update_stock_levels_on_woocommerce_site(self, mock_wc_api, mock_frappe):
        ...
        # Set up mock return values for servers
        ...
        # --- UPDATE: Mock the Cache ---
        # We simulate that the cache returns None (Cache Miss), forcing the sync to happen.
        ...
        # Expected quantity is 15 (5 from Warehouse A + 10 from Warehouse B)
        ...
        # --- UPDATE: Assert Cache was Set ---
        # We expect set_value to be called twice (once for each successful sync)
        self.assertEqual(mock_cache.set_value.call_count, 2)
        # Verify the first call set the correct values
        mock_cache.set_value.assert_any_call("wc_stock_sync:1:some_item_code", 15)
    ...
    def test_update_stock_levels_on_woocommerce_site_variant(self, mock_wc_api, mock_frappe):
        ...
        # --- UPDATE: Assert Cache was Set ---
        mock_cache.set_value.assert_called_with("wc_stock_sync:101:variant_item_code", 15)

    @patch("woocommerce_softland.tasks.stock_update.frappe.db.get_all")
    @patch("woocommerce_softland.tasks.stock_update.frappe.enqueue")
